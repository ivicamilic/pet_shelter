...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $target_path)) {
            // Optimize image: resize to max 800x800px and compress // Optimiziraj sliku: promeni veličinu na max 800x800px i kompresuj
            ...
            if (array_key_exists($img_type, $allowed_types)) {
                switch ($img_type) {
                    ...
                    case IMAGETYPE_PNG:
                        ...
                        break;
                    case IMAGETYPE_WEBP:
                        $src_img = imagecreatefromwebp($img_path);
                        break;
                    case IMAGETYPE_GIF:
                ...
                if ($src_img) {
                    ...
                    switch ($img_type) {
                        case IMAGETYPE_JPEG:
                            imagejpeg($dst_img, $img_path, 70); // Quality 70 for JPEG // Kvalitet 70 za JPEG
                            break;
                        case IMAGETYPE_PNG:
                            imagepng($dst_img, $img_path, 9); // Compression 9 for PNG // Kompresija 9 za PNG
                            break;
                        case IMAGETYPE_WEBP:
                            imagewebp($dst_img, $img_path, 85); // Quality 85 for WEBP // Kvalitet 85 za WEBP
                            break;
                        case IMAGETYPE_GIF:
                            imagegif($dst_img, $img_path); // GIF with no quality param // GIF bez parametra kvaliteta
                            break;
                    }
...
document.getElementById('image-upload').addEventListener('change', function() {
    ...
    // Show image preview // Prikaži pregled slike
    ...
    if (this.files && this.files[0]) {
        ...
        reader.onload = function(e) {
            imageSection.innerHTML = `
                <label class="form-label"><?php echo $L['selected_image'] ?? 'Selected Image'; ?></label>
                <div>
                ...
                <div class="input-group mt-2">
                    ...
                    <input type="text" class="form-control" id="file-name" readonly placeholder="<?php echo $L['no_file_chosen'] ?? 'No file chosen'; ?>" value="${fileName}">
                </div>
            `;
            // Re-attach the event listener after DOM update // Ponovo priključi event listener nakon ažuriranja DOM-a
            document.getElementById('file-upload-button').addEventListener('click', function() {
                document.getElementById('image-upload').click();
            });
        };
        ...
        // Reset to initial state if no file selected // Resetuj na početnu stanje ako nema fajla
        imageSection.innerHTML = `
            <label class="form-label"><?php echo $L['image'] ?? 'Image'; ?></label>
            <div class="input-group">
                <input type="file" class="form-control d-none" id="image-upload" name="image" accept="image/*">
